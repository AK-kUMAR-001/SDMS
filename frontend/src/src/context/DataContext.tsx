...
interface DataContextType {
  ...
  loading: boolean;
  addUser: (user: Omit<User, 'id' | 'status'>, password: string) => Promise<boolean>;
...
export const DataProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchCertificates = useCallback(async () => {
    try {
        const data = await localDB.getCertificates();
  ...
  useEffect(() => {
    if (currentUser) {
        setLoading(true);
        Promise.all([fetchUsers(), fetchCertificates()]).finally(() => setLoading(false));
    } else {
        setUsers([]);
        setCertificates([]);
        setLoading(false);
    }
  }, [currentUser, fetchUsers, fetchCertificates]);
  ...
  const getAuditLogs = useCallback(async (page: number, pageSize: number) => {
    try {
        const result = await localDB.getAuditLogs(page, pageSize);
