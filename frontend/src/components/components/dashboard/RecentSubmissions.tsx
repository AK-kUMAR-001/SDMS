    ...
    <Card>
      ...
      <CardContent>
        <div className="space-y-6">
          {console.log('RecentCertificates:', recentCertificates)}
          {recentCertificates.map((cert) => (
            <div key={cert.id} className="flex items-center">
              {console.log('Current Certificate:', cert)}
              <Avatar className="h-9 w-9">
                <AvatarImage src={`https://i.pravatar.cc/150?u=${cert.userName}`} alt="Avatar" />
                <AvatarFallback>{cert.userName.charAt(0)}</AvatarFallback>
              </Avatar>
              <div className="ml-4 space-y-1">
                <p className="text-sm font-medium leading-none">{cert.userName}</p>
