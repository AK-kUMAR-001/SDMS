...
const fs = require('fs');
const path = require('path');
const { sequelize, User, Role, Permission, Certificate, AuditLog, LeaderboardEntry } = require('./models/index.cjs');
const app = express();
...
const server = app.listen(PORT, HOST, () => {
  try {
    ...
    console.log(`Server is running on ${addr.family} ${addr.address}:${addr.port}`);
  } catch (e) {
